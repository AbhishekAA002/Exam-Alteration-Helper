...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<script>
        function updateTimer() {
            ...
            var formatted_time = ('Time : ')+('0' + hours).slice(-2) + ':' + ('0' + minutes).slice(-2) + ':' + ('0' + seconds).slice(-2);
...
<div class="ann" style="background-color:transparent;border-radius: 15px;float: left;box-shadow: 3px 3px 3px 3px #CCC;color:white">
  ...
  $sql = "SELECT * FROM tempdata   LIMIT 1";
  $result = mysqli_query($conn, $sql);

  // Fetch the data from the first row and first column
  if (mysqli_num_rows($result) > 0) {
      $row = mysqli_fetch_assoc($result);
      $first_column_data = $row['state'];
      $first_column_dat = $row['value2'];
    
      // Display the data in an H1 tag with blue color style
      echo "<div style='color: green;text-align:center;font-size:28px'>"."State value : " . $first_column_data. " "."<br> Pressure : " . $first_column_dat. "</div>";
  } else {
      echo "No data found.";
  }
...
<div style="background-color: trasparent;margin-left: 30px;float: right;width: 550px;border-radius: 15px;box-shadow: 3px 3px 3px 3px #CCC;color:white">
 <p style="font-size:30px;text-align: center;"> Temparature Box </p>
 <hr>
 ...
 <?php
  ...
  if (mysqli_num_rows($result) > 0) {
      $row = mysqli_fetch_assoc($result);
      $first_column_data = $row['value1'];
     
      // Display the data in an H1 tag with blue color style
      echo "<div style='text-align: center;font-size: 28px;padding:10px'>".$first_column_data."</div>";
  } else {
      echo "No data found.";
  }
